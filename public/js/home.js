...
$(".create-heat-map").on("submit", function(event){
    event.preventDefault();
    $("#visualizationWell").empty();
    var user = $("#usernameExpensesHeatMap").val().trim();
    heatMapQuery(user);
});
...
/////////////
  ...
  function heatMapQuery(user) {
    // The AJAX function uses the URL of our API to GET the data associated with it (initially set to localhost)
    $.ajax("/api/expenses/" + user, {
      type: "GET",
    })
    .then(function(expenseData) {
      console.log("heat map query run")
      createHeatMap(expenseData);
    });
  }
...
function createHeatMap(d3Object) {
